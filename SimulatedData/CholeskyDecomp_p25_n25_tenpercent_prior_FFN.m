...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Simulation combination: n25, p25, sparsity = tenpercent
...
%save('PaperCholeskyDecomp_Sigma_true_p25_n25_tenpercent_xmat.mat','sigma_true');

%save the omega matrix for R
%save('PaperCholeskyDecomp_Omega_true_p25_n25_tenpercent_xmat.mat','omega_true');

%set the number of replications for the simulation
reps = 100;

    y_true_n25_p25 = cell([reps,1]);

    optimalJ_n25_p25_tenpercent = cell([reps,1]);
    minK_n25_p25_tenpercent = cell([reps,1]);
    Final_AIC_n25_p25_tenpercent = cell([reps,1]);
    

 x_matrix_n25_p25 =  cell([reps,1]);
...
Z_two_cell_iters = cell([reps,1]);
    ...
    MLE_ald_n25_p25 = cell([reps,1]);

    MLE_ev_n25_p25 = cell([reps,1]);
    MLE_stable_n25_p25 = cell([reps,1]);
   % power_n25_p25 = cell([reps,1]);
...
for iters = 1:reps
    ...
    %Save the true y
    y_true_n25_p25{iters} = y_true;
     
   
    [x_matrix, MLE_ald, MLE_ev, MLE_stable] = xfunctions_farfromNormal(p, y_true);
    
    x_matrix_n25_p25{iters} = x_matrix;
    MLE_ev_n25_p25{iters} = MLE_ev;
    MLE_stable_n25_p25{iters} = MLE_stable;
    MLE_ald_n25_p25{iters} = MLE_ald;
...
N_points);
    ...
    optimalJ_n25_p25_tenpercent{iters} = optimalJ;
    minK_n25_p25_tenpercent{iters} = minK;
    Final_AIC_n25_p25_tenpercent{iters} = Final_AIC;
...
save('CholeskyDecomp_p25_n25_tenpercent_prior_FFN.mat');
