  ...
  num_elements = 1;
   ...
   entropy_loss_n300_p100_twopercent = zeros(num_elements,1, reps);
    ...
    Frobenius_norm_covariance_n300_p100_twopercent = zeros(num_elements,1, reps);
     ...
     TP_n300_p100_twopercent = zeros(num_elements,1, reps);
    TN_n300_p100_twopercent = zeros(num_elements,1, reps);
    FP_n300_p100_twopercent = zeros(num_elements,1, reps);
    FN_n300_p100_twopercent = zeros(num_elements,1, reps);
...
[SP, SE, MCC, total_time, Omega_Bayes_est,Frobenius_norm_precision,entropy_loss,...
...
inverse_variance_prior_reduced_cell_iters{iters},...
    mean_prior_reduced_cell_iters{iters},...
       Z_red_cell_iters{iters}, Z_two_cell_iters{iters},...
    initial_value_cell_iters{iters}, capital_A,capital_B,  tau, zeta_squared,num_VB_samples);
...
%save all of the data for each hyperparameter setting.
   ...
   total_time_n300_p100_twopercent( iters) = total_time; %this is the
        %total time it took to run the sampling 
 entropy_loss_n300_p100_twopercent( iters) = entropy_loss; 
   bounded_loss_n300_p100_twopercent( iters) = bounded_loss; 
   Frobenius_norm_precision_n300_p100_twopercent( iters) = Frobenius_norm_precision; 


    edge_matrix_n300_p100_twopercent{ iters} = edge_matrix_estimate;

   Omega_Bayes_est_n300_p100_twopercent{ iters} = Omega_Bayes_est;
    mean_Z_Bayes_est_n300_p100_twopercent{ iters} = mean_Z_Bayes_est;
    TP_n300_p100_twopercent( iters) = TP;
    TN_n300_p100_twopercent( iters) = TN;
    FP_n300_p100_twopercent( iters) = FP;
    FN_n300_p100_twopercent( iters) = FN;
...
save('CholeskyDecomp_p100_n300_twopercent_VB_76to100.mat', '-v7.3');
