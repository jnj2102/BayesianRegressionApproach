  ...
  %  run('run_to_compile_and_test.m')
   ...
   [num_elements, ~] = size(c_tune_list);
    ...
    MCC_matrix_bglasso =  zeros(num_elements,1, reps);
     TP_bglasso_n300_p100_AR2 = zeros(num_elements,1, reps);
    TN_bglasso_n300_p100_AR2 = zeros(num_elements,1, reps);
    FP_bglasso_n300_p100_AR2 = zeros(num_elements,1, reps);
    FN_bglasso_n300_p100_AR2 = zeros(num_elements,1, reps);
...
%save all of the data for each hyperparameter setting.
   ...
   total_time_n300_p100_AR2(A_index, iters) = total_time; %this is the
        %total time it took to run the sampling 
 entropy_loss_n300_p100_AR2(A_index, iters) = entropy_loss; 
   bounded_loss_n300_p100_AR2(A_index, iters) = bounded_loss; 
   Frobenius_norm_precision_n300_p100_AR2(A_index, iters) = Frobenius_norm_precision; 
   Frobenius_norm_covariance_n300_p100_AR2(A_index, iters) = Frobenius_norm_covariance; 


    edge_matrix_n300_p100_AR2{A_index, iters} = edge_matrix_bglasso;

   Omega_Bayes_est_n300_p100_AR2{A_index, iters} = Omega_Bayes_est;
    Sigma_Bayes_est_n300_p100_AR2{A_index, iters} = Sigma_Bayes_est;
    mean_Z_Bayes_est_n300_p100_AR2{A_index, iters} = mean_Z_Bayes_est;
    TP_bglasso_n300_p100_AR2(A_index, iters) = TP_bglasso;
    TN_bglasso_n300_p100_AR2(A_index, iters) = TN_bglasso;
    FP_bglasso_n300_p100_AR2(A_index, iters) = FP_bglasso;
    FN_bglasso_n300_p100_AR2(A_index, iters) = FN_bglasso;
...
save('CholeskyDecomp_p100_n300_AR2_HS_96to100.mat', '-v7.3');
