...
[~, ~, ~,edge_matrix_estimate,...
   ~, ~, ~, ~, Omega_Bayes_est, total_time,mean_Z_Bayes_est, ...
   ~,~,...
    ~,~, Sigma_Bayes_est] = BayesianNonpar_CholeskyDecomp_MCMC_SpikeSlab_mem(n,p,...
     data_matrix_std, omega_true, sigma_true, F_mat_cell, g_vec_cell,...
     W_mat_cell, q_vec_cell,...
knot_vector_cell, index_1_cell, index_2_cell, inverse_variance_prior_reduced_cell,...
 mean_prior_reduced_cell,...
Z_red_cell, Z_two_cell,...
initial_value, capital_A, capital_B, tau, zeta_squared);

    
  save('RealData_CholeskyDecomp_SS_Wille.mat', '-v7.3');